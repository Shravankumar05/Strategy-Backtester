...
class DataPreprocessor:
    ...
    def preprocess_ohlcv_data(self, df: pd.DataFrame, symbol: str, strict_mode: bool = True) -> Tuple[pd.DataFrame, DataQualityReport]:
        ...
        try:
            df_cleaned = self._validate_basic_structure(df, symbol, report)
            df_cleaned = self._handle_missing_values(df_cleaned, symbol, report)
            df_cleaned = self._validate_data_types(df_cleaned, symbol, report)
            df_cleaned = self._handle_outliers(df_cleaned, symbol, report)
            df_cleaned = self._validate_ohlcv_relationships(df_cleaned, symbol, report)
            self._final_validation(df_cleaned, symbol, report)
            
            if strict_mode and (report.has_errors() or report.missing_values.get('Open', 0) > 0 or report.ohlcv_violations > 0):
                error_msg = f"Data quality errors for {symbol}: {'; '.join(report.errors) if report.errors else 'Data quality issues detected'}"
                raise DataError(error_msg)
            
            self.logger.info(f"Preprocessing completed for {symbol}. "
                           f"Warnings: {len(report.warnings)}, Errors: {len(report.errors)}")
            
            return df_cleaned, report
            
        except Exception as e:
            report.errors.append(f"Preprocessing failed: {str(e)}")
            if strict_mode:
                raise DataError(f"Failed to preprocess data for {symbol}: {str(e)}")
            return df, report
    ...
    def _validate_basic_structure(self, df: pd.DataFrame, symbol: str, report: DataQualityReport) -> pd.DataFrame:
        ...
        if not isinstance(df.index, pd.DatetimeIndex):
            try:
                ...
                report.warnings.append("Converted index to DatetimeIndex")
            except Exception as e:
                report.errors.append(f"Could not convert index to DatetimeIndex: {str(e)}")
    ...
    def _handle_missing_values(self, df: pd.DataFrame, symbol: str, report: DataQualityReport) -> pd.DataFrame:
        ...
        if rows_after != rows_before:
            report.warnings.append(f"Dropped {rows_before - rows_after} rows with missing values")
                ...
                if current_type != expected_type:
                    try:
                        ...
                        report.data_type_issues.append(
                            f"Could not convert {column} to {expected_type}: {str(e)}"
                        )
        
        return df
    
    def _handle_outliers(self, df: pd.DataFrame, symbol: str, report: DataQualityReport) -> pd.DataFrame:
        price_columns = ['Open', 'High', 'Low', 'Close']
        
        threshold = self.outlier_threshold
        if len(df) <= 10 and df['Open'].max() > 1000:
            threshold = 2.0
        ...
        for column in price_columns + ['Volume']:
            ...
            if outlier_count > 0:
                report.outliers_detected[column] = int(outlier_count)
                report.warnings.append(
                    f"Detected {outlier_count} outliers in {column} "
                    f"(Z-score > {threshold})"
                )
                
                if column in price_columns:
                    upper_bound = mean_val + (3 * std_val)
                    lower_bound = max(0.01, mean_val - (3 * std_val))  # Ensure positive prices
                    ...
                    report.warnings.append(
                        f"Volume outliers detected but not modified (might be legitimate)"
                    )
    ...
    def _validate_ohlcv_relationships(self, df: pd.DataFrame, symbol: str, report: DataQualityReport) -> pd.DataFrame:
        ...
        if high_violations > 0:
            ...
            report.warnings.append(f"Fixed {high_violations} High < max(Open,Close) violations")
        
        if low_violations > 0:
            violations += low_violations
            mask = df['Low'] > df[['Open', 'Close']].min(axis=1)
            df.loc[mask, 'Low'] = df.loc[mask, ['Open', 'Close']].min(axis=1)
            report.warnings.append(f"Fixed {low_violations} Low > min(Open,Close) violations")
        
        high_low_violations = (df['High'] < df['Low']).sum()
        if high_low_violations > 0:
            violations += high_low_violations
            mask = df['High'] < df['Low']
            high_values = df.loc[mask, 'High'].copy()
            df.loc[mask, 'High'] = df.loc[mask, 'Low']
            df.loc[mask, 'Low'] = high_values
            report.warnings.append(f"Fixed {high_low_violations} High < Low violations")
        
        negative_volume = (df['Volume'] < 0).sum()
        if negative_volume > 0:
            violations += negative_volume
            df.loc[df['Volume'] < 0, 'Volume'] = 0
            report.warnings.append(f"Fixed {negative_volume} negative volume values")
        ...
        for col in ['Open', 'High', 'Low', 'Close']:
            ...
            if negative_prices > 0:
                ...
                df = df[df[col] > 0]
                report.warnings.append(f"Removed {negative_prices} rows with non-positive {col} values")
    ...
    def _final_validation(self, df: pd.DataFrame, symbol: str, report: DataQualityReport) -> None:
        ...
        if nan_counts.sum() > 0:
            report.errors.append(f"NaN values remain after cleaning: {nan_counts.to_dict()}")
        
        if not df.index.is_monotonic_increasing:
            report.warnings.append("Date index is not sorted - fixing")
            df.sort_index(inplace=True)
        
        duplicate_dates = df.index.duplicated().sum()
        if duplicate_dates > 0:
            report.warnings.append(f"Found {duplicate_dates} duplicate dates - removing")
